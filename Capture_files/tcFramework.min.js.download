...
return value;}
return val;}
function parseUrl(str){var url;var dotCount;if(typeof URL==="function"){url=new URL(str);}else{url=customParseUrl(str)}
url.domain=(url.host.indexOf(".")>=0&&(dotCount=url.host.split(".").length)>=3)?url.host.split(".").slice(1).join("."):url.host;if(url.domain.indexOf("co.uk")===0){url.domain=url.host;}
url.subDomainSupport=dotCount>3;return url;}
function customParseUrl(str){var result;var m=str.match(/^(https?:)\/\/([^\/:]+)(:?(\d*))/);var protocol=m[1];var host=m[2];var port=m[4]?m[4]:protocol==="http:"?"80":protocol==="https:"?"443":null;var origin=protocol+"//"+host;if(port!==""&&port!=="80"&&port!=="443"){origin+=":"+port;}
result={origin:origin,href:str,protocol:protocol,host:host,port:port};return result;}
...
var url=urls.conversationTranscriptURL+"/getTranscript";var conversationTranscriptMode=CHM.getChat()&&CHM.getChat().getRule()?CHM.getChat().getRule().conversationTranscriptMode:null;var conversationTranscriptDays=CHM.getChat()&&CHM.getChat().getRule()?CHM.getChat().getRule().conversationTranscriptDays:null;var data=prepareBaseBodyForAsyncChatDataRequest();data.businessUnitID=CHM.getBusinessUnitID();if(!CHM.getChat().isAsyncReconnect()||CHM.getChat().getCustMsgCnt()>0){data.excludeEngagementID=CHM.getChatID();}
...
this.post(url,data,null,false,callbackFcn,null);};RM91.prototype.hasActiveConversations=function(){function callbackFcn(response){if(response.responseStatus<200||response.responseStatus>299){let12("Error returned for active conversation request, status code "+response.responseStatus,true);CONVM.reset();}else{if(response.data){var responseData=JSON.parse(response.data);var conversationsLength=responseData.conversations.length;setConversationActive(conversationsLength>0);if(conversationsLength>0){CONVM.setLastBusinessUnitAndAgentGroup(responseData.conversations[0]["lastBusinessUnitID"],responseData.conversations[0]["lastAgentGroupID"]);}else{CONVM.reset();}}else{let12("Empty response returned for active conversation request",true);CONVM.reset();}}
